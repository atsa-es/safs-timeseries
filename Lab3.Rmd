...
coef = broom::tidy(ss_ar)
coef = coef[grep("pred", coef$term),]
plot(1:length(Temp), coef$estimate,col="black", ylab="Fit", type="l", lwd=2, ylim = c(55,100), main="AR model", xlab="")
lines(1:length(Temp), coef$estimate+coef$std.error)
lines(1:length(Temp), coef$estimate-coef$std.error)

coef = broom::tidy(ss_rw)
coef = coef[grep("pred", coef$term),]
lines(1:length(Temp), coef$estimate, col="blue", lwd=2)
lines(1:length(Temp), coef$estimate+coef$std.error, col="blue")
lines(1:length(Temp), coef$estimate-coef$std.error, col="blue")
